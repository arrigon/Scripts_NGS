...
foreach my $acc (@accs){ #iterate accession
  ...
  foreach $i (0..$len){
	...
	$coords{$acc}{$cnt}{"start"} = $i;
      ...
      if($startflag == 1 && $Dcntdown < $limD){ #still in informative part of chunck, just tracking its length
	$Dcntdown++;
	}

      if($startflag == 1 && $Dcntdown == $limD){ #close chunck if it gets too long (defined using limD)
	$startflag = 0; #close chunck!
	$coords{$acc}{$cnt}{"stop"} = $i;
	}

      if($startflag == 1 && $i == $len){ #close chunck if we end-up at the other side of alnmt
	$coords{$acc}{$cnt}{"stop"} = $i;
	}

      } else { #we are in a gap, start counting N's and cut if needed
	$Ncntdown++;
