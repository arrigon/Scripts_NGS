...
##### Assembly of DMPS datasets, using MIRA, Ref-guided assembly from SINGLE-END reads
### Performed steps:
...
# # # foreach my $spec (@dddd){
  ...
  if(($loadCPU + 2) < 30 and $freeRAM > 0) {
    #### Automated loader
    my $t = threads->new(\&runMIRA, $realname, $readfolder, $file1, $refseq);
    push(@threads, $t);
    $startedthreads++;
    sleep(25);
...
### Routines
sub runMIRA {
  my ($realname, $readfolder, $file1, $refseq) = ($_[0], $_[1], $_[2], $_[3]);
