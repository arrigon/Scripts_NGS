...
foreach my $db (@dbs) {
    my $tsvfile = $params{out_dir}.$params{id}.'_'.$db.'.tsv';
    my $cmd = PROG_DIR.PROG_NAME.' bwasw -A -f '.$tsvfile.(exists $params{S} ? ' -S '.$params{S} : '').(exists $params{z} ? ' -z '.$params{z} : '').(exists $params{T} ? ' -T '.$params{T} : '').' '.DB_DIR.DBS->{$db}->{db}.' '.$params{f};
    $time = &runCmd($cmd);
...
if(exists $params{no_seq_out}) {
    ...
    my %ids_keep;
    foreach my $db (@dbs_retain) {
        &parseTsvFiles($data{$db}->{tsv},\%ids_keep) if(exists $data{$db}->{tsv});
    }
...
sub checkForDbFiles {
    ...
    my $status = 1;
    foreach my $ext (@exts) {
	$test = -e DB_DIR.DBS->{$db}->{db}.'conta.'.$ext;
	print "XXXXXXXXXXXXX $ext = $test\n";
        unless(-e DB_DIR.DBS->{$db}->{db}.'conta.'.$ext) {
            $status = 0;
            last;
        }
    }
