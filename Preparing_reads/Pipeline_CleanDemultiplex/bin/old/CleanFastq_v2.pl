...
my $command = "fastq-multx -B param/barcodes.txt data.in/$file1.gz data.in/$file2.gz -m 2 -o data.mult/%.R1.$num.fastq.gz -o data.mult/%.R2.$num.fastq.gz";
print "Running:\n$command\n\n";
system($command);
...
## demultiplex files, just this is needed actually
my$command = "fastq-mcf -l 75 -o data.clean/clean.$file1.fastq -o data.clean/clean.$file2.fastq param/Primers.fas data.mult/$file1 data.mult/$file2"
print "Running:\n$command\n\n";
system($command);
