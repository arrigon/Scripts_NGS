...
foreach my $i (keys(%corresp)){
  ...
  foreach my $j (keys(%tmp)){
    ...
    my$command = "perl bin/RepairSyncFastq.pl $outmultiplex/$file1 $outmultiplex/$file2 $outrepaired/";
    print "Running: $command\n\n";
    system($command);
    ...
    my$command = "./bin/fastq-mcf -p 10 -F param/Contaminants.fas -q $phred -k 0 -l $minlen -o $outqualclean/clean.$file1 -o $outqualclean/clean.$file2 param/Primers.fas $outrepaired/$file1 $outrepaired/$file2 > $outlogsfolder/qualclean/Clean.$file1.log";
    print "Running: $command\n\n";
    system($command);
    }
  }
