...
#### run FLASH
my $command = "bin/flash $input1 $input2 -o $bsn1 -d $outfolder";
print "$command\n";
system($command);


#### stitch together unmatched reads
%store1 = %{IOfastq("$outfolder/$bsn1.notCombined\_1.fastq", "fqr")};
%store2 = %{IOfastq("$outfolder/$bsn1.notCombined\_2.fastq", "fqr")};

open(OUT1, ">$outfolder/$bsn1.stitch");
...
### put all of these reads into a same file
my $command = "cat $outfolder/$bsn1.extendedFrags.fastq $outfolder/$bsn1.stitch > $outfolder/$bsn1";
print "$command\n";
system($command);


### put all of these reads into a same file
my $command = "rm $outfolder/$bsn1.extendedFrags.fastq $outfolder/$bsn1.stitch $outfolder/$bsn1.his* $outfolder/$bsn1.notCom*";
print "$command\n";
system($command);
